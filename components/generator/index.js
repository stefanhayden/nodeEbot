...
var Generator = function Generator(config) {
  ...
  // Keep track of all our word stats
	this.dictionary = {}; // This is a new object I'm using to generate sentences in a more efficient manner than the wordpairs array.
	this.startwords = []; // Words that start a sentence.
	this.stopwords = []; // Stop words.
	this.hashtags = []; // Automatically detect any hashtags that we use.
	this.wordpairs = []; // This is where we'll store our dictionary of word pairs
	this.popularKeywords = []; // Track popular keywords that the bot may be interested in based on our tweet history.
}

// Build out initial word pair dictionary for creating
// semi-intelligent sounding sentences pseudo Markov chains.
Generator.prototype.buildCorpus = function(content) {
	var countWords = 0;
	...
	// console.log('STARTWORDS: ', this.startwords.length);
...
Generator.prototype.checkExists = function(value) {
	if (!value) {
		//return '';
	} else {
		return value;
...
Generator.prototype.checkSentenceEnd = function(word) {
...
Generator.prototype.choice = function (array) {
...
Generator.prototype.choosePairs = function (firstWord, secondWord) {
	var allResults = [];
	var resultWordPair;
	var getResult;
...
Generator.prototype.cleanContent = function(content) {
	var cleaned = content;
...
Generator.prototype.makeTweet = function (min_length) {
...
Generator.prototype.searchObject = function (array, prop, value) {
...
Generator.prototype.getKeywords = function(word) {
...
Generator.prototype.makeSentenceFromKeyword = function(replystring) {  
...
Generator.prototype.twitterFriendly = function (reply, username) {
	var new_tweet;
	if (reply) {
		username = '@' + username + ' ';
	} else {
...
*
*/
Generator.prototype.chooseRandomPair = function (obj) {
...
Generator.prototype.findWordPair = function(string) {
...
Generator.prototype.attachHashtag = function(tweetlength) {
	var gethashtag;
	var x = Math.random(); // Generate random number to determine whether we add a hashtag or not
	if (x <= this.config.personality.addHashtags) {
		...
		if (this.config.personality.ignoredHashtags.indexOf(gethashtag.toLowerCase()) !== -1) {
...
Generator.prototype.attachEmoji = function(tweetlength) {
	var emoji;
	var emojis = [
		" 💩💩💩",
		" 😍😍",
	...
	if (x <= this.config.personality.addEmojis) {
		// Pick a random emoji from our array
