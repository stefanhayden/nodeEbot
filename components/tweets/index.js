...
var Tweets = function Tweets(generator, config) {
	this.config = config;
...
Tweets.prototype.mapInterests = function() {
	this.config.personality.robotInterests.forEach(function(element) {
...
Tweets.prototype.watchStream = function (mode) {
	...
	if (this.config.settings.monitorStream) {
		this.client.stream('user', function(stream){
			...
			stream.on('data', function(tweet) {
				...
				self.config.settings.lastTweetReceivedTime = Math.floor(Date.now() / 1000);
				...
				if (tweet.delete || tweet.friends) {
					...
					if (self.config.settings.canFavoriteTweets) {
			...
			if (mode === 'destroy') {
				self.config.settings.monitorStream = false;
				//stream.destroy();
				//return;
			}
...
Tweets.prototype.countReplies = function(user) {
	var countReplies = this.config.settings.trackReplies.reduce(function(n, val) {
			return n + (val === user);
	}, 0);
...
Tweets.prototype.checkReply = function(tweet) {
	var checkTweet = tweet.text.toLowerCase();
	var myUsername = '@' + this.config.settings.robotName.toLowerCase();
	...
	if (this.config.settings.randomReplies && x <= config.settings.randomRepliesChance && checkTweet.indexOf(myUsername) == -1 && (typeof tweet.retweeted_status === "undefined")) {
		...
		if (replyUsername.toLowerCase() !== this.config.settings.robotName.toLowerCase() && replyCount < 5) {
			this.config.settings.trackReplies.push(replyUsername); // Add user to our reply tracker so we don't spam them too much. 
	...
	if (checkTweet.indexOf(myUsername) != -1 && (typeof tweet.retweeted_status == "undefined")) {
		...
		for (var i = 0; i < checkMentions.length; i++) {
					if (checkMentions[i].substring(0, 1) == "@") {
						...
						if (checkMentions[i].toLowerCase() != '@' + this.config.settings.robotName.toLowerCase()) {
		...
		if (replyUsername.toLowerCase() !== this.config.settings.robotName.toLowerCase() && replyCount < 5) {
			...
			if (this.config.personality.otherBots.indexOf(replyUsername.toLowerCase()) != -1) {
				console.log('User \'' + replyUsername +'\' is in our bot list. Temporarily limiting replies to this user.');
				//this.config.settings.trackReplies.push(replyUsername,replyUsername,replyUsername,replyUsername,replyUsername,replyUsername);
			}
...
Tweets.prototype.checkInterests = function (tweet) {
	if (tweet.id !== null) {
		var tweetID;
		var tweetUsername;
		var tweetText;
		...
		if (this.config.settings.robotName.toLowerCase() === tweetUsername.toLowerCase()) {
...
Tweets.prototype.checkIgnored = function(username) {
	if (this.config.settings.ignoredUsers.indexOf(username.toLowerCase()) != -1) {
		return true;
	} else {
		return false;
	}
...
Tweets.prototype.checkStream = function() {
	var curTime = Math.floor(Date.now() / 1000);   
	// It's been longer than around 120 seconds since we've seen a tweet. Let's restart the stream.
	// console.log('[' + utils.currentTime() + '] Debug - lastTweetReceivedTime: ', this.config.settings.lastTweetReceivedTime);
	if (curTime - this.config.settings.lastTweetReceivedTime >= 3600) {
...
// Send a tweet!
Tweets.prototype.postNewTweet = function(send_msg) {
	if (this.config.settings.postTweets) {
		this.client.post('statuses/update', {status: send_msg},  function(error, tweet, response){
			if(error) {
...
Tweets.prototype.writeReply = function(username, replyID, replytext) {
	...
	if (this.checkIgnored(username)) {
		...
		var myReply = self.generator.makeSentenceFromKeyword(replytext);
		...
		if (typeof myReply !== 'undefined') {
			...
			replyTweet = self.generator.twitterFriendly(true, username); 
		}
			
			console.log('\nReplying to user @' + username + ':');
			console.log(replyTweet);
			if (self.config.settings.respondReplies) self.sendReply(replyTweet,replyID); 
		}, (randomDelay * 1000));
	}
},
...
Tweets.prototype.sendReply = function(send_msg, replyID) {
	this.client.post('statuses/update', {status: send_msg, in_reply_to_status_id: replyID},  function(error, tweet, response){
		if(error) {
...
Tweets.prototype.getFollowers = function() {
	var self = this;
	...
	this.client.get('followers/list', {count: 3}, function(error, followers, response){
		if(error) {
		...
		if(!error) {
			...
			self.client.post('friendships/create', {screen_name: newFollower},  function(error, tweet, response){
